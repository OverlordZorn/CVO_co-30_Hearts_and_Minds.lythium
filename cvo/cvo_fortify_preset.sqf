...
[{
	...
	private _condition1 = if ((_unit) getVariable ["isEngineer", false]) then {true}
		else {"You're not an engineer" remoteExec ["hint", _unit];false};

	private _condition2 = if (50 > ((_unit) distance cvo_fortify_vehicle)) then {true}
...
_cvo_Fort_refillVehicle = [
	...
	{25 > (btc_create_object distance cvo_fortify_vehicle) && [west] call ace_fortify_fnc_getBudget < 5000}		// Condition for action to be shown:
] call ace_interact_menu_fnc_createAction;

// Here we define where we want this action that we created to be attached to
[
	cvo_fortify_vehicle,					// Object the action should be assigned to
